...
.campanha-produto {
  ...
  background: #fff; border-radius: 14px; padding: 10px 20px; width: 100%; box-sizing: border-box;
...
.ganhadores-logo-topo img { height: 100px; width: auto; max-width: 22vw; object-fit: contain; filter: drop-shadow(0 6px 14px rgba(0,0,0,0.5)); }
.ganhadores-premio {
...
function montarSlidesCompletos(chaveEscolhida) {
  ...
  const blocosTimes = SUPERVISORES.flatMap(slidesBlocoSupervisor);
  // Onde a apresentação continua quando alguém sai do "modo equipe" pelo
...
let slidesAtuais = [];
let atual = 0;
let emModoEquipe = false;

function ir(i) {
  if (i >= slidesAtuais.length) {
    if (emModoEquipe) continuarAposEquipe();
    return;
  }
  if (i < 0) return;
